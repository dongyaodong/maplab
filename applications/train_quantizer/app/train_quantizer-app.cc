...
DECLARE_int32(lc_number_of_vocabulary_words);
DECLARE_int32(lc_num_descriptors_to_train);
DECLARE_int32(lc_product_quantization_num_components);
DECLARE_int32(lc_product_quantization_num_dim_per_component);
DECLARE_int32(lc_product_quantization_num_words);

DEFINE_string(vi_map_folder, "", "Path to a VI-map used for training.");
...
void LoadBinaryFeaturesFromDataset(
  ...
  // Get the descriptor-length. This function is corrected from origin one
  ...
  for (const vi_map::MissionId& mission_id : all_mission_ids) {
    ...
    VLOG(0) << " mission_descriptors " << mission_descriptors.rows()
            << ", mission_descriptors col" << mission_descriptors.cols();
    int old_size = all_descriptors->cols();
    /*all_descriptors.conservativeResize(
        Eigen::NoChange, old_size + mission_descriptors.cols());*/
    all_descriptors->conservativeResize(
        mission_descriptors.rows(), old_size + mission_descriptors.cols());
...
int main(int argc, char** argv) {
  ...
  ros::init(argc, argv, "train_quantizer");
  ros::NodeHandle nh;
  ...
  CHECK_NE(FLAGS_lc_projection_matrix_filename, "")
      << "You have to provide a filename to load the projection matrix.";
